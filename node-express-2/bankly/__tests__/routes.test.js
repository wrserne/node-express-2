...
describe("POST /auth/login", function() {
  ...
  // TESTS BUG #5: Incorrectly calling User.authenticate without await
  test("should deny access with incorrect username/password", async function() {
    const response = await request(app)
      .post("/auth/login")
      .send({
        username: "u1",
    ...
    expect(response.body).toEqual({ message: "Invalid username/password" });
  });
});
...
describe("GET /users", function() {
  ...
  // TESTS BUG #1: Remove unnecessary username and password arguments
  test("should list all users with admin access", async function() {
    const response = await request(app)
      .get("/users")
...
describe("GET /users/[username]", function() {
  ...
  // TESTS BUG #2: User not found should return 404
...
describe("PATCH /users/[username]", function() {
  ...
  // TESTS BUG #3: Update non-existing user should return 404
  ...
  // TESTS BUG #4: Incorrect use of 'await' while updating user info
  test("should return updated user info if user updates own info", async function() {
    const response = await request(app)
      .patch("/users/u1")
      .send({ _token: tokens.u1, first_name: "new-fn" });
    expect(response.statusCode).toBe(200);
    expect(response.body.user).toEqual({
      username: "u1",
      first_name: "new-fn",
  ...
  // TESTS BUG #6: Deleting a user that doesn't exist should return 404
  test("should return 404 if cannot find user to delete", async function() {
    const response = await request(app)
  ...
  // TESTS BUG #7: Incorrect use of 'await' while deleting user
